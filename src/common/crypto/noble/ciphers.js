...
export var ctr = wrapCipher({ blockSize: 16, nonceLength: 16 }, function ctr2(key, nonce) {
  bytes(key);
  bytes(nonce, BLOCK_SIZE2);
...
export var gcm = wrapCipher({ blockSize: 16, nonceLength: 12, tagLength: 16 }, function gcm2(key, nonce, AAD) {
  bytes(key);
  bytes(nonce);
  if (AAD !== void 0)
    bytes(AAD);
